...
#     Rescaled uu and vv by omega - there may be a mistake somewhere, so I'm making a copy below to try to find it
    vv_rhs = (vv*alpha_rhs)/alpha + (3*vv*chi_rhs)/(2*chi)
    for i in range(DIM):
        vv_rhs += (uu*beta_rhsU[i]*omega_dD[i])/(omega) -((vv*betaU[i]*alpha_dD[i])/alpha) - (3*vv*betaU[i]*chi_dD[i])/(2*chi) - (alpha**2*chi*uu*contractedGammahatU[i]*omega_dD[i])/(omega) - (uu*betaU[i]*alpha_rhs*omega_dD[i])/(omega*alpha) - (3*uu*betaU[i]*chi_rhs*omega_dD[i])/(2*omega*chi) -alpha**2*chi*contractedGammahatU[i]*uu_dD[i] - (betaU[i]*alpha_rhs*uu_dD[i])/alpha - (3*betaU[i]*chi_rhs*uu_dD[i])/(2*chi) + beta_rhsU[i]*uu_dD[i] + 2*betaU[i]*vv_dD[i] + vv*betaU_dD[i][i]
        for j in range(DIM):
            # PART 1:
            vv_rhs += (3*uu*betaU[i]*betaU[j]*chi_dD[j]*omega_dD[i])/(2*omega*chi) - (alpha**2*uu*rfm.ghatUU[i][j]*chi_dD[j]*omega_dD[i])/(2*omega) + (uu*betaU[i]*betaU[j]*alpha_dD[i]*omega_dD[j])/(omega*alpha) + (alpha*chi*uu*rfm.ghatUU[i][j]*alpha_dD[i]*omega_dD[j])/(omega) + (2*uu*betaU[i]*betaU[j]*omega_dD[i]*omega_dD[j])/(omega)**2 - (2*alpha**2*chi*uu*rfm.ghatUU[i][j]*omega_dD[i]*omega_dD[j])/(omega)**2 + (3*betaU[i]*betaU[j]*chi_dD[j]*uu_dD[i])/(2*chi) - alpha**2*rfm.ghatUU[i][j]*chi_dD[j]*uu_dD[i]/2 + (betaU[i]*betaU[j]*alpha_dD[i]*uu_dD[j])/alpha + alpha*chi*rfm.ghatUU[i][j]*alpha_dD[i]*uu_dD[j] + vv*betaU[i]*rfm.GammahatUDD[j][i][j] - ((uu*betaU[i]*omega_dD[j]*betaU_dD[j][i])/(omega)) - betaU[i]*uu_dD[j]*betaU_dD[j][i] - (uu*betaU[i]*omega_dD[i]*betaU_dD[j][j])/(omega) - betaU[i]*uu_dD[i]*betaU_dD[j][j] - (uu*betaU[i]*betaU[j]*omega_dDD[i][j])/(omega) + (alpha**2*chi*uu*rfm.ghatUU[i][j]*omega_dDD[i][j])/(omega) - betaU[i]*betaU[j]*uu_dDD[i][j] + alpha**2*chi*rfm.ghatUU[i][j]*uu_dDD[i][j]
            for k in range(DIM):
                vv_rhs += -((uu*betaU[i]*betaU[j]*omega_dD[i]*rfm.GammahatUDD[k][j][k])/(omega)) - betaU[i]*betaU[j]*uu_dD[i]*rfm.GammahatUDD[k][j][k]
#     vv_rhs = 0
#     for i in range(DIM):
#         vv_rhs += -((vv*betaU[i]*alpha_dD[i])/alpha) - (alpha**2*chi*uu*contractedGammahatU[i]*omega_dD[i])/(omega) -alpha**2*chi*contractedGammahatU[i]*uu_dD[i] + 2*betaU[i]*vv_dD[i] + vv*betaU_dD[i][i]
#         for j in range(DIM):
#             # PART 1:
#             vv_rhs += (uu*betaU[i]*betaU[j]*alpha_dD[i]*omega_dD[j])/(omega*alpha) + (alpha*chi*uu*rfm.ghatUU[i][j]*alpha_dD[i]*omega_dD[j])/(omega) + (2*uu*betaU[i]*betaU[j]*omega_dD[i]*omega_dD[j])/(omega)**2 - (2*alpha**2*chi*uu*rfm.ghatUU[i][j]*omega_dD[i]*omega_dD[j])/(omega)**2 + (betaU[i]*betaU[j]*alpha_dD[i]*uu_dD[j])/alpha + alpha*chi*rfm.ghatUU[i][j]*alpha_dD[i]*uu_dD[j] + vv*betaU[i]*rfm.GammahatUDD[j][i][j] - ((uu*betaU[i]*omega_dD[j]*betaU_dD[j][i])/(omega)) - betaU[i]*uu_dD[j]*betaU_dD[j][i] - (uu*betaU[i]*omega_dD[i]*betaU_dD[j][j])/(omega) - betaU[i]*uu_dD[i]*betaU_dD[j][j] - (uu*betaU[i]*betaU[j]*omega_dDD[i][j])/(omega) + (alpha**2*chi*uu*rfm.ghatUU[i][j]*omega_dDD[i][j])/(omega) - betaU[i]*betaU[j]*uu_dDD[i][j] + alpha**2*chi*rfm.ghatUU[i][j]*uu_dDD[i][j]
#             for k in range(DIM):
#                 vv_rhs += -((uu*betaU[i]*betaU[j]*omega_dD[i]*rfm.GammahatUDD[k][j][k])/(omega)) - betaU[i]*betaU[j]*uu_dD[i]*rfm.GammahatUDD[k][j][k]

        # Add Kreiss-Oliger dissipation to the RHSs: (taken from nrpytutorial/Tutorial-BaikalETK.ipynb)
        #thismodule = "KO_Dissipation"
        diss_strength = sp.sympify(0)/1000 #par.Cparameters("REAL", thismodule, "diss_strength", default_KO_strength)

        uu_dKOD = ixp.declarerank1("uu_dKOD")
        vv_dKOD = ixp.declarerank1("vv_dKOD")
        #uu_dKOD = ixp.zerorank1()
        #vv_dKOD = ixp.zerorank1()
        #for i in range(DIM):
            #uu_dKOD[i] = uu_dD[i]
            #vv_dKOD[i] = vv_dD[i]
        for i in range(DIM):
            uu_rhs += diss_strength*uu_dKOD[i]*rfm.ReU[i] # ReU[k] = 1/scalefactor_orthog_funcform[k]
            vv_rhs += diss_strength*vv_dKOD[i]*rfm.ReU[i] # ReU[k] = 1/scalefactor_orthog_funcform[k]
