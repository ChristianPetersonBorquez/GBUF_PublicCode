...
def ScalarWaveCurvilinear_RHSs_MetricComponents():
    ...
    global gg_rhs,gp_rhs,alpha_rhs,chi_rhs,omega_rhs,vet_rhsU, psi_rhsD
    ...
    gg_rhs = (gp+psiD[0])/sp.sympify(2)

    #gp_rhs = rfm.ghatUU[0][0]*(gp-psiD[0])/sp.sympify(2) + rfm.ghatUU[1][1]*psiD_dD[1][1] + rfm.ghatUU[2][2]*psiD_dD[2][2] -contractedGammahatU[0]*(gp-psiD[0])/sp.sympify(2) -contractedGammahatU[1]*psiD[1] -contractedGammahatU[2]*psiD[2] + gp_dD[0]/sp.sympify(2) + psiD_dD[0][0]/sp.sympify(2)
    #psi_rhsD[0] = rfm.ghatUU[0][0]*(gp-psiD[0])/sp.sympify(2) + rfm.ghatUU[1][1]*psiD_dD[1][1] + rfm.ghatUU[2][2]*psiD_dD[2][2] -contractedGammahatU[0]*(gp-psiD[0])/sp.sympify(2) -contractedGammahatU[1]*psiD[1] -contractedGammahatU[2]*psiD[2] - gp_dD[0]/sp.sympify(2) - psiD_dD[0][0]/sp.sympify(2)
    ...
    #gp_rhs = gp_dD[0] +(gp-psiD[0])/rfm.xx[0]
    #psi_rhsD[0] = -psiD_dD[0][0] +(gp-psiD[0])/rfm.xx[0]

    psi_rhsD[1] = ( gp_dD[1] + psiD_dD[0][1] )/sp.sympify(2)
    psi_rhsD[2] = ( gp_dD[2] + psiD_dD[0][2] )/sp.sympify(2)
...
#     Rescaled uu and vv by omega - there may be a mistake somewhere, so I'm making a copy below to try to find it
    ...
    #diss_strength = sp.sympify(0)/1000  #FIX DISSIPATION
    ...
    #for i in range(DIM):
        ...
        #for j in range(DIM):
            #psi_rhsD[i] += diss_strength*psiD_dKOD[i][j]*rfm.ReU[j] # ReU[k] = 1/scalefactor_orthog_funcform[k]
