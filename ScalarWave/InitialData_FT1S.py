...
def SphericalGaussian(CoordSystem="Cartesian",default_time=0,default_sigma=3,default_center=1/4):
    ...
    #uu_ID = (+(r-wavespeed*time)/r*sp.exp(-(r-wavespeed*time)**2/(2*sigma**2)) + (r+wavespeed*time)/r*sp.exp(-(r+wavespeed*time)**2/(2*sigma**2))) #+ sp.sympify(1) # original
    #uu_ID = 1*r*r*omega_ID*omega_ID*sp.exp(-(r*r-center*center)**2/(4*sigma**4)) # spherically symmetric
    #uu_ID = 1*r*r*omega_ID*omega_ID*sp.exp(-(r*r-center*center*(1+4*sp.sqrt(1-costheta**2)))**2/(4*sigma**4)) # axially symmetric
    uu_ID = 1*r*r*omega_ID*omega_ID*sp.exp(-(r*r-center*center*(1+4*1/sp.sqrt(1+tanphi**2)))**2/sigma**4) # not spherically symmetric
    vv_ID = sp.sympify(0) #sp.diff(uu_ID, time)
    psi_ID = ixp.zerorank1()
    #psi_ID[0] = sp.diff(uu_ID, r)
    psi_ID[0] = 2*omega_ID*omega_ID*sp.exp(-(r*r-center*center*(1+4*1/sp.sqrt(1+tanphi**2)))**2/sigma**4)*r-4*omega_ID*omega_ID*sp.exp(-(r*r-center*center*(1+4*1/sp.sqrt(1+tanphi**2)))**2/sigma**4)*r**3*(r*r-center*center*(1+4*1/sp.sqrt(1+tanphi**2)))/sigma**4
    psi_ID[1] = sp.sympify(0)
    #psi_ID[2] = sp.sympify(0)
    psi_ID[2] = -8*omega_ID*omega_ID*center**2*sp.exp(-(r*r-center*center*(1+4*1/sp.sqrt(1+tanphi**2)))**2/sigma**4)*r**2*(1+tanphi**2)*tanphi*(r*r-center*center*(1+4*1/sp.sqrt(1+tanphi**2)))/(sigma**4*(1+tanphi**2)**(-3/2))
    #psi_ID[2] = sp.diff(uu_ID, sp.atan(tanphi))
